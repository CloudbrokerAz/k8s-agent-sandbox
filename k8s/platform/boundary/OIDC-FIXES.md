...
Users accessing Boundary via `https://boundary.hashicorp.lab` were getting OIDC errors during the OAuth2 callback phase.

## Root Causes and Fixes

### 1. Keycloak Hostname Configuration (KC_HOSTNAME_URL)

**Problem**: Keycloak was configured with `KC_HOSTNAME=keycloak.hashicorp.lab` which caused it to advertise HTTP URLs in the OIDC discovery document, but users access via HTTPS ingress.

**Error**:
```
oidc: issuer did not match the issuer returned by provider, expected "http://keycloak.hashicorp.lab/realms/agent-sandbox" got "https://keycloak.hashicorp.lab/realms/agent-sandbox"
```

**Fix**: Use `KC_HOSTNAME_URL=https://keycloak.hashicorp.lab` instead of `KC_HOSTNAME=keycloak.hashicorp.lab`
...
- name: KC_HOSTNAME_URL
  value: "https://keycloak.hashicorp.lab"
...
**Fix**: Update the OIDC auth method issuer to `https://keycloak.hashicorp.lab/realms/agent-sandbox`

**File**: `k8s/platform/boundary/scripts/configure-oidc-auth.sh`
```bash
OIDC_ISSUER="https://keycloak.hashicorp.lab/realms/agent-sandbox"
```

### 3. Boundary Controller Network Access to HTTPS Keycloak

**Problem**: Boundary controller needs to validate the OIDC provider by fetching the discovery document. It must reach `https://keycloak.hashicorp.lab:443`.

**Fix**: Update Boundary controller `hostAliases` to point `keycloak.hashicorp.lab` to the ingress controller service IP (which handles TLS termination).
...
# Then use $INGRESS_IP for keycloak.hashicorp.lab hostAlias
```
...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
  ...
  -addext "subjectAltName=DNS:keycloak.hashicorp.lab,DNS:keycloak.keycloak.svc.cluster.local"
```
...
kubectl exec -n boundary <controller-pod> -c boundary-controller -- \
  wget --no-check-certificate -O - 'https://keycloak.hashicorp.lab/realms/agent-sandbox/.well-known/openid-configuration'
```
